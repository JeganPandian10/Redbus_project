...
with state_col:
    ...
    conn = connection()
    if conn:
        state_query = "SELECT DISTINCT `state` FROM redbus_data"
...
if selected_state != "--- Select State ---":
    conn = connection()
    if conn:
        from_query = f"SELECT DISTINCT `From` FROM redbus_data WHERE `state` = '{selected_state}'"
        to_query = f"SELECT DISTINCT `To` FROM redbus_data WHERE `state` = '{selected_state}'"
...
with filter_col:
    ...
    with filter_col3:
        ...
        bus_type_query = "SELECT DISTINCT `bustype` FROM redbus_data"
        conn = connection()
        if conn:
